...
function UI:_BuildTitleBar(f)
    ...
    -- Bouton « panier de vente » : ouvre le composeur (sinon accessible seulement par
    -- Alt+clic droit sur un objet du sac).
    local sellBtn = CreateFrame("Button", nil, f)
    sellBtn:SetSize(20, 20)
    sellBtn:SetPoint("LEFT", gearBtn, "RIGHT", 6, 0)
    sellBtn:SetNormalTexture("Interface\\Icons\\INV_Misc_Coin_01")
    sellBtn:SetHighlightTexture("Interface\\Buttons\\UI-Common-MouseHilight", "ADD")
    sellBtn:SetScript("OnClick", function()
        if TS.SellComposer then TS.SellComposer:Open() end
    end)
    sellBtn:SetScript("OnEnter", function(b)
        GameTooltip:SetOwner(b, "ANCHOR_RIGHT")
        GameTooltip:SetText(L["Sell to Guild Economy"])
        GameTooltip:AddLine(L["Open the sell basket"], 0.7, 0.7, 0.7, true)
        GameTooltip:Show()
    end)
...
function UI:Build()
    ...
    f:SetClampedToScreen(true); f:SetFrameStrata("HIGH"); f:SetFrameLevel(10); f:Hide()
